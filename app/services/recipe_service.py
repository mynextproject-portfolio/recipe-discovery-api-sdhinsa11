from typing import List, Optional
from app.models.recipe import Recipe, RecipeCreate
from app.storage.abstract import RecipeStorageInterface
from app.services.mealdb_service import MealDBService

class RecipeService:
    def __init__(self, storage: RecipeStorageInterface):
        self.storage = storage
        self.mealdb_service = MealDBService()
    
    def get_all_recipes(self) -> List[dict]:
        recipes = self.storage.get_all()
        # Add source field to internal recipes
        for recipe in recipes:
            recipe["source"] = "internal"
        return recipes
    
    def get_recipe_by_id(self, recipe_id: int) -> Optional[dict]:
        recipe = self.storage.get_by_id(recipe_id)
        if recipe:
            recipe["source"] = "internal"
        return recipe
    
    def create_recipe(self, recipe: RecipeCreate) -> dict:
        recipe_data = self.storage.create(recipe.model_dump())
        recipe_data["source"] = "internal"
        return recipe_data
    
    def update_recipe(self, recipe_id: int, recipe: RecipeCreate) -> Optional[dict]:
        recipe_data = self.storage.update(recipe_id, recipe.model_dump())
        if recipe_data:
            recipe_data["source"] = "internal"
        return recipe_data
    
    def delete_recipe(self, recipe_id: int) -> bool:
        return self.storage.delete(recipe_id)
    
    def search_recipes(self, query: str) -> List[dict]:
        """Search both internal recipes and external MealDB API"""
        if not query:
            return []
        
        # Search internal recipes
        internal_recipes = self.storage.search(query)
        for recipe in internal_recipes:
            recipe["source"] = "internal"
        
        # Search external MealDB API
        external_recipes = self.mealdb_service.search_meals_sync(query)
        
        # Combine results - internal recipes first
        combined_results = internal_recipes + external_recipes
        
        return combined_results

def get_recipe_service(storage: RecipeStorageInterface) -> RecipeService:
    """Factory function for recipe service"""
    return RecipeService(storage)